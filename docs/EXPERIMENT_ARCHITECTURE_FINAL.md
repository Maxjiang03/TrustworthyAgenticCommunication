...
**Status:** **Sealed specification. All fifteen Part G gates PASS; sealed at v0.5 (`805425e`) and v0.6 (`cdf185d`); a v0.7 reseal is owed (ADR 0044) and Part H step 7 has NOT run.** This is the specification the implementation follows. No statement here may be described as "proven by specification"; every implementation assumption is enumerated in §F.4 with the gate that must confirm it.

*Dated correction, 2026-08-07 (ADR 0044). This line read "**Implementation candidate — NOT sealable** … **No confirmatory experiment, no sealing, and no v0.5** until every in-scope Part G gate passes." That precondition was satisfied — every in-scope gate passed on the pilot corpus — and the seal was taken twice, so the sentence now reads as a live prohibition on work already done under it. Three further status statements in this document (in §E's preamble, §H's, and the closing line) said the same thing and carry the same correction by reference. Nothing else changed: no frozen row, no §E.4 cell, no predicate and no gate criterion has been edited by any reseal. The reseal to v0.7 is owed because a pre-run audit found the apparatus could not execute step 7 and would have measured two things wrongly, NOT because anything in this specification changed.*
